...
function run() {
  ...
  assert.ok(bioStarts.some((dt) => dt.hour === 20));
  assert.ok(bioStarts.some((dt) => dt.hour === 23));
  assert.ok(!bioStarts.some((dt) => dt.hour === 17));
  assert.ok(rocketStarts.some((dt) => dt.hour === 17));
  assert.ok(!rocketStarts.some((dt) => dt.hour === 9));
