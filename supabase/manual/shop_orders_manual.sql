-- shop_orders already exists in production (verified 2026-07-07).
-- Run only if list_tables does not show public.shop_orders.
...
CREATE TABLE IF NOT EXISTS public.shop_orders (
  ...
  CONSTRAINT shop_orders_status_allowed CHECK (
    status IN ('pending', 'paid', 'processing', 'shipped', 'delivered', 'refunded', 'cancelled')
  )
);

CREATE INDEX IF NOT EXISTS shop_orders_status_created_idx ON public.shop_orders (status, created_at DESC);
CREATE INDEX IF NOT EXISTS shop_orders_email_idx ON public.shop_orders (lower(email));
